...
1. **Store** — `~/.claude/mymem/` ไฟล์จริงอยู่ที่เดียวต่อเครื่อง
2. **Symlink** — `memory/` ของทุก project ชี้มาที่นี่ → ทุก session ใช้ pool เดียว
3. **Sync** — two-way `rsync` ผ่าน Tailscale กรองตาม scope

## ติดตั้ง

MyMem เป็น **Rust single binary** (pure std ไม่มี dep ภายนอก) บนแต่ละเครื่อง:
...
`install.sh` จะ `cargo build --release` → ติดตั้ง binary ที่ `~/.local/bin/mymem`, ตั้ง alias, symlink ทุก project เข้า pool, ติดตั้ง auto-sync hooks, ถาม peer

> ไม่มี cargo แต่ arch เดียวกัน (Apple Silicon)? build ที่เครื่องหนึ่งแล้ว `scp target/release/mymem อีกเครื่อง:.local/bin/`

แต่ละ release แนบ prebuilt binary สำหรับ **macOS** (arm64 + Intel) และ **Linux x86_64** (ที่ WSL2 ใช้) — ดู [Releases](https://github.com/MegaWiz-Dev-Team/mymem/releases) · `install.sh` จะรัน **preflight** ก่อน ถ้าขาด prerequisite จะ exit โดยไม่แตะอะไรเลยและบอกว่าต้องลงอะไรบ้าง
...
> ⚠️ **รัน Claude Code *ใน* WSL2** · mymem จัดการ `~/.claude/mymem` ใน WSL home · Windows desktop app เก็บ memory ที่ `C:\Users\<you>\.claude` (`/mnt/c/Users/<you>/.claude` จาก WSL) ซึ่ง **ไม่ได้** bridge ไว้ ถ้าติดตั้ง hooks จาก WSL จะไปลงผิด `settings.json` · `install.sh` จะเตือนถ้าเจอเคสนี้
...
# หรือ:  export MYMEM_PEER="you@mac-b,you@mac-c"
```
...
memory แต่ละก้อนมี `metadata.origin: <hostname>` = เครื่องที่เขียนมันก้อนแรก MyMem stamp ตอนเขียนใหม่ก่อน push (ของเดิม grandfather เป็น `?`) ดูได้ที่ `mymem status`/`doctor` (บรรทัด `origins:`), `mymem list` (โชว์ origin ต่อ shared memory), และ dashboard panel **Origins** + tooltip ของ node (`· from <เครื่อง>`)

## วิธีใช้

### ปกติ: ไม่ต้องทำอะไร

หลัง `./install.sh` MyMem ทำงาน **อัตโนมัติทุก session** ผ่าน hooks — เปิด Claude = ดึง memory ล่าสุด + autolink; Claude เขียน memory = push ออกหลังจบ turn

### งานที่ทำเอง

```bash
mymem share project_firestore_envs   # ตั้งเป็น shared (default = local) + push
mymem local debug_scratch_today      # ถอด tag → local (ไม่ sync)
mymem list                           # ดูว่าอันไหน shared / local
mymem sync                           # sync เอง (hook ทำให้แล้ว)
mymem doctor                         # health + token footprint + สิ่งที่ควรแก้
mymem dash && open ~/.claude/mymem/dashboard.html   # dashboard (graph + token)
mymem link                           # ใส่ project ปัจจุบันเข้า pool เดี๋ยวนี้
```
...
| คำสั่ง | ทำอะไร |
|---|---|
| `mymem sync` | push + pull เฉพาะ `scope: shared` + regen index |
| `mymem push` / `pull` | ทิศเดียว (shared เท่านั้น) |
| `mymem share <id>` | ตั้ง memory เป็น shared + push |
| `mymem local <id>` | ถอด tag → local (ไม่ sync) |
| `mymem list` | shared vs local |
| `mymem reserve <repo> [<version>\|--patch\|--minor\|--major] [desc]` | จอง version ก่อนขึ้น feature/issue ใหม่ (ดู [จอง version](#จอง-version-)); ไม่ใส่ version/flag → minor ถัดไป |
| `mymem reservations [repo] [--all]` | ดูรายการจองที่ active (จัดกลุ่มตาม repo + owner/อายุ/desc) พร้อมเตือน collision |
| `mymem release <repo> <version>` | ปล่อยคืนเมื่องานขึ้นเสร็จแล้ว |
| `mymem status` | store / counts / peer |
| `mymem help` | ดูคำสั่งทั้งหมด (หรือ `-h` / `--help`) |
| `mymem start` | autolink + sync (SessionStart hook เรียก) |
| `mymem link` | symlink project ปัจจุบันเข้า pool |
| `mymem doctor [--check]` | health + actions (`--check` = เงียบถ้าไม่มีปัญหา ใช้ใน hook) |
| `mymem compact-index [types] [--off]` | ลด token ของ index ที่โหลดทุก session — เก็บเฉพาะ Tier-0 ใน `MEMORY.md` ที่เหลือย้ายไป `MEMORY.full.md` (ดู [Token footprint](#token-footprint-)) |
| `mymem fix-links [--apply]` | ซ่อม `[[links]]` ที่เสียซึ่งมีปลายทางชัดเจนตัวเดียว (dry-run ถ้าไม่ใส่ `--apply`) |
| `mymem autolink` | symlink project ปัจจุบันเข้า pool แบบเงียบ (ถ้า link แล้วไม่พิมพ์อะไร ใช้ใน hook) |
| `mymem dash` | สร้าง `dashboard.html` แบบ static (graph + token viz) |
| `mymem serve [--port N]` | dashboard **กดสั่งงานได้** บน `127.0.0.1` — คลิก node = toggle shared/local, ปุ่ม sync |

### Interactive dashboard

`mymem serve` รัน HTTP server เล็กๆ บน localhost (pure std) แล้วเปิด browser — ต่างจาก `dash` static ตรงที่**สั่งงานได้**:
...
MyMem sync **เฉพาะที่ตั้งใจสื่อสารข้ามเครื่อง** ไม่ใช่ทั้งหมด คุมด้วย frontmatter:
...
- toggle: `mymem share <id>` / `mymem local <id>`

## Token footprint 🪙

มี **อย่างเดียวที่ always-on**: `MEMORY.md` (index) — Claude Code โหลดเข้า context ทุก session ส่วนที่เหลือ (เนื้อ memory ทั้ง pool) เป็น **on-demand**: `mymem search` อ่านไฟล์ตรงๆ ดังนั้น catalog เต็มๆ กิน **0 token** จนกว่าจะ query จริง

`MEMORY.md` มี 1 บรรทัดต่อ 1 memory → โตตามจำนวน memory พอถึงหลักร้อยตัวมันกิน context ประจำเยอะ `mymem doctor` จะขึ้น 🔴 เมื่อเกิน ~12k tokens มี 2 คำสั่งช่วยให้มันเล็ก:

### `mymem compact-index` — Tier-0 split

```bash
mymem compact-index            # MEMORY.md = เฉพาะ user + feedback
mymem compact-index user feedback project   # กำหนด Tier-0 เอง
mymem compact-index --off      # คืนค่า index เต็ม
```
...
### `mymem fix-links` — ซ่อม `[[links]]` ที่เสีย

```bash
mymem fix-links            # dry-run: list link เสีย + ตัวที่จะแก้
mymem fix-links --apply    # เขียนแก้เฉพาะตัวที่ชัดเจน
```
...
```bash
mymem reserve Mimir "OCR retry queue"      # → MINOR ถัดไป (เช่น 0.5.0)
mymem reserve Mimir --patch "fix index"    # → PATCH ถัดไป
mymem reserve heimdall 2.4.0 "tool-call"   # → ระบุ version ตรง ๆ
mymem reservations                          # ใครจองอะไร (⚠ เตือน collision)
mymem release Mimir 0.5.0                    # ปล่อยคืนเมื่องานขึ้นเสร็จ
```
...
ต้องมี: macOS หรือ Linux/WSL2 (x86_64), **Rust/cargo** (build — WSL2 ต้องมี `build-essential` ด้วยสำหรับ linker `cc`) หรือ prebuilt binary ของ platform นั้นจาก Releases, shell rc (`~/.zshrc` บน macOS / `~/.bashrc` บน WSL2 — `install.sh` เลือกให้), rsync+ssh, python3 (install.sh merge hooks), **Tailscale** ทั้งสองเครื่อง (Mac app บน macOS / Linux client บน WSL2), **Remote Login** เปิดฝั่งปลายทาง, **SSH key auth** ระหว่างเครื่อง, **peer** ที่ `~/.claude/mymem.conf`
